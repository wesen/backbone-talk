...
require_once(dirname(__FILE__).'/third-party/'.MINIFY_JS);
	...
	function render($return = false) {
		...
		if (!$this->_fromCache) {	
			...
			$this->_parsedSource = $this->parseFile($file, $context);
                        if ($this->minify) {
                          $this->_parsedSource = JSMin::minify($this->_parsedSource);
                        }
		}
		...
		if ($return) {
			return $this->_parsedSource;
                ...
                if ($this->_fromCache) {
                  ...
                  $hash = md5($this->_parsedSource);
                  header('Cache-Control: max-age='.$this->cacheMaxAge);
